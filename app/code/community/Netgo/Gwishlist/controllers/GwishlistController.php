...
class Netgo_Gwishlist_GwishlistController extends Mage_Core_Controller_Front_Action
{
        ...
        if (Mage::helper('netgo_gwishlist/gwishlist')->getUseBreadcrumbs()) {
            if ($breadcrumbBlock = $this->getLayout()->getBlock('breadcrumbs')) {
                ...
                $breadcrumbBlock->addCrumb(
                    'gwishlists',
                    array(
                        'label' => Mage::helper('netgo_gwishlist')->__('My Wishlist'),
                        'link'  => '',
                    )
                );
            }
	...
	public function removeitemAction()
	{
		...
		//die('stopnnn');	
		Mage::getSingleton('core/session')->addSuccess($this->__('Product removed from your Wishlist'));
		$url = Mage::getBaseUrl().'gwishlist/gwishlist/';
		$this->_redirectUrl($url);
	}
	...
	public function updateAction()
	{
	   ...
	   try {
			...
			Mage::getSingleton('core/session')->addSuccess($this->__('Wishlist updated'));			
       	}
		catch (Exception $e) {			
			Mage::getSingleton('core/session')->addError($e->getMessage());
		}
	...
	public function addallitemAction()
	{
		...
		foreach($data as $gwishlistProdcut) {
			...
			try {
				...
				Mage::getSingleton('core/session')->addSuccess($_product->getName().' '.$this->__('has been added from Wishlist to Cart'));
			 	$cart->save(); 
			 	$url = Mage::getBaseUrl().'checkout/cart/';
	...
	public function updategwishlistAction() 
	{
		...
		if ($post_data) {
			try {
				...
				Mage::getSingleton('core/session')->addSuccess($this->__('Product updated in Wishlist')); 
				$url = Mage::getBaseUrl().'gwishlist/gwishlist/';
			}
			catch (Exception $e) {
				$url = Mage::getBaseUrl();
